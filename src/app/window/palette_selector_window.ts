...
import { DocumentData, DrawLineTypeID, FillAreaTypeID, Layer, VectorLayer } from '../document_data'
...
export class PaletteSelectorWindow {
  ...
  getCurrentLayerTargetColorRef(ctx: DocumentContext): Vec4 {
    ...
    switch (this.currentTargetID) {
      ...
      case PaletteSelectorWindowButtonID.adjustmentMode:
        return ctx.document.paletteColors[this.currentPaletteIndex].color
    }
  }
  ...
  updatePaletteItems(ctx: DocumentContext) {
    ...
    for (const layoutArea of this.itemAreas) {
      ...
      if (paletteColorIndex > ctx.document.paletteColors.length) {
        break
      }

      const paletteColor = ctx.document.paletteColors[paletteColorIndex]
