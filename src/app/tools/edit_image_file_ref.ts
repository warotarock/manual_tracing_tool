...
import { RenderImage } from '../render/render3d'
import { SubToolContext } from '../context/subtool_context'
import { LatticePointEditTypeID, Tool_Transform_Lattice, TransformModifyType, TransformType } from './transform_lattice'
...
class Tool_Transform_ReferenceImage extends Tool_Transform_Lattice {
  ...
  protected prepareLatticePoints(ctx: SubToolContext): boolean { // @override
    ...
    this.calculateImageLatticePoints(
      ctx.currentImageFileReferenceLayer.imageResource.image,
  ...
  protected processTransform(ctx: SubToolContext) { // @override
    ...
    const image = ifrLayer.imageResource.image
    ...
    if (this.transformModifyType == TransformModifyType.one) {
      ...
      this.calculateImageLatticePoints(
        ctx.currentImageFileReferenceLayer.imageResource.image,
        ctx.currentImageFileReferenceLayer.adjustingLocation,
        ctx.currentImageFileReferenceLayer.adjustingRotation,
        ctx.currentImageFileReferenceLayer.adjustingScale
      )
    ...
    vec3.set(ifrLayer.adjustingScale, scaleX, scaleY, 0.0)
