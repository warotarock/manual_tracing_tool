...
class SubjoinProcessingState {
  ...
  targetLine_OrderedPoints: List<VectorPoint> = null;
  targetLine_SegmentIndex = HitTest_Line.InvalidIndex;

  subjoinLine_OrderedPoints: List<VectorPoint> = null;

  newLine: VectorStroke = null;
  deleteLines = new List<VectorStroke>();
}
...
export class Tool_ScratchLineDraw extends Tool_ScratchLine {
  ...
  private getLineOverlappingInfo(sourcePoints: List<VectorPoint>, source_StartIndex: int, targetPoints: List<VectorPoint>, target_StartIndex: int, minDistanceRange: float): LineOverlappingInfo {
  ...
  private createSubjoinedLine(topPoints: List<VectorPoint>, topPonts_OverlappingInfo: LineOverlappingInfo, followingPoints: List<VectorPoint>, followingPoints_OverlappingInfo: LineOverlappingInfo, resamplingUnitLength: float, subjoinToAfter: boolean): VectorStroke {
    ...
    if (subjoinedIndex - 2 >= 0 && subjoinedIndex + 4 <= newPoints.length - 1) {
      let resampledPoins = new List<VectorPoint>();
  ...
  private executeProcessLine(state: SubjoinProcessingState, subjoinLine: VectorStroke, subjoinToAfter, env: ToolEnvironment) {
