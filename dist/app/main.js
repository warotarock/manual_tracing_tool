...
import { StringIsNullOrEmpty, List } from 'base/conversion';
import { DocumentData, DocumentFileType, Layer, LayerTypeID, VectorLayer, FillAreaTypeID, DrawLineTypeID, } from 'base/data';
import { MainToolID, ToolContext, ToolEnvironment, ToolDrawingEnvironment, DrawPathContext, DrawPathStep, DrawPathOperationTypeID, TempVirtualLayer, TempVirtualLayerTypeID, DrawPathModeID, OperationUnitID } from 'base/tool';
import { CanvasWindow } from 'renders/render2d';
import { Command_Layer_AddVectorLayerToCurrentPosition, Command_Layer_AddVectorLayerReferenceLayerToCurrentPosition, Command_Layer_AddGroupLayerToCurrentPosition, Command_Layer_AddPosingLayerToCurrentPosition, Command_Layer_AddImageFileReferenceLayerToCurrentPosition } from 'commands/edit_layer';
import { NewLayerTypeID } from 'app/view.class';
import { ImageResource, ModelResource } from 'posing3d/posing3d_view';
import { App_Event } from './event';
import { Platform } from 'platform/platform';
import { DocumentLogic } from 'logics/document';
import { CommandHistory } from 'base/command';
export var MainProcessStateID;
...
var App_Main = /** @class */ (function (_super) {
    ...
    function App_Main() {
        ...
        _this.drawPathContext = new DrawPathContext();
        _this.lazy_DrawPathContext = new DrawPathContext();
    ...
    App_Main.prototype.processLoadingSystemResources = function () {
        ...
        if (this.localSetting.lastUsedFilePaths.length == 0
            ...
            var newDocument = new DocumentData();
    ...
    App_Main.prototype.setHeaderDefaultDocumentFileName = function () {
        var fileName = DocumentLogic.getDefaultDocumentFileName(this.localSetting);
    ...
    App_Main.prototype.collectDrawPaths = function () {
        ...
        {
            var drawPathStep = new DrawPathStep();
    ...
    App_Main.prototype.collectDrawPaths_CollectVirtualLayerRecursive = function (result, layers) {
        for (var i = 0; i < layers.length; i++) {
            var layer = layers[i];
            var vLayer = new TempVirtualLayer();
            ...
            if (layer.isMaskedByBelowLayer) {
                ...
                var virtualGroup_vLayer = new TempVirtualLayer();
                ...
                while (nextIndex < layers.length) {
                    ...
                    var next_vLayer = new TempVirtualLayer();
    ...
    App_Main.prototype.collectDrawPasths_getCompositeOperationString = function (layer) {
        ...
        if (!activeLayerBufferDrawn) {
            ...
            for (var i = drawPathContext.startIndex; i <= drawPathContext.endIndex; i++) {
                ...
                if (VectorLayer.isVectorLayer(layer)) {
                    ...
                    this.drawForeground(viewKeyFrameLayer, this.toolContext.document, false, drawPathContext.isModalToolRunning);
                }
            }
        }
        if (bufferCanvasWindow != null) {
    ...
    App_Main.prototype.lazyDraw_Process = function (drawPathContext) {
    ...
    App_Main.prototype.onModalWindowClosed = function () {
        if (this.currentModalDialogID == this.ID.layerPropertyModal) {
            ...
            var operationUnitID = this.getRadioElementIntValue(this.ID.operationOptionModal_operationUnit, OperationUnitID.linePoint);
    ...
    App_Main.prototype.onNewLayerCommandOptionModal = function () {
        ...
        if (newLayerType == NewLayerTypeID.vectorLayer) {
            ...
            var command = new Command_Layer_AddVectorLayerToCurrentPosition();
